...
export default function ParcelTimeline({ parcel: parcel }: Props) {
  // Reverse logs → latest first
  const reversedLogs = [...(parcel.statusLogs || [])].reverse();
            ...
            <li key={idx} className="relative flex gap-3 mb-6">
              ...
              >
                {isCurrentStatus(idx) ? (
                  ...
                  <span className="w-2 h-2 bg-white dark:bg-gray-200 rounded-full"></span>
                )}
              </span>
